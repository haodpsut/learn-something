...
class GT_FID(nn.Module):
    def __init__(self, seq_vocab_size, seq_emb_dim=64, lstm_hidden=128, gcn_hidden=64, fused_dim=192, num_classes=2):
        super(GT_FID, self).__init__()
        self.seq_emb = nn.Embedding(seq_vocab_size, seq_emb_dim, padding_idx=0)
        self.lstm = nn.LSTM(seq_emb_dim, lstm_hidden, batch_first=True)
        self.gcn1 = GCNConv(1, gcn_hidden)
        self.gcn2 = GCNConv(gcn_hidden, gcn_hidden)
        self.fc_fuse = nn.Linear(lstm_hidden + gcn_hidden, fused_dim)
    ...
    def forward(self, seqs, seq_lens, graph_batch):
        ...
        h_gcn = torch.zeros((graph_batch.num_graphs, x.size(1)), device=device)
